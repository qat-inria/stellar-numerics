...
def max_trace_distance_precision(
    ...
    if isinstance(from_profile.state, PureCVState) and isinstance(to_profile.state, PureCVState):
        if protocol is Protocol.deterministic:
            return max_trace_distance_precision_pure_pure_det(
                from_profile=from_profile, to_profile=to_profile, nb_copies=nb_copies
            )
        elif protocol is Protocol.probabilistic:
            pass
...
def max_trace_distance_precision_pure_pure_det(
...
def max_trace_distance_precision_pure_pure_prob(
    from_profile: StellarProfile[U], to_profile: StellarProfile[V], nb_copies: int
) -> float:  # or None, error
    ...
    # need to supply the rank of input
    # change parameters
